...
class ASV_BIM:
    ...
    def _load_enrollment_embedding(self, spk_id):
        enroll_candidates = glob.glob(os.path.join(self.enroll_path, f"{spk_id}*.flac"))
    ...
    def forward(self, target_spk=None, test_utterance=None):
        ...
        if target_embedding is None:
            print(f"No enrollment embedding for {target_spk}")
            return None
        ...
        for _ in range(self.steps):
            ...
            grad = torch.autograd.grad(loss_coss, adv, retain_graph=False)[0]
